...
var SolveWithBacktracking = function (steps, id, cells, gridCells, options) {
  ...
  var cell = options.bestFirst
    ? GetBestCell(cells, gridCells, options)
    : cells[id];
  ...
  while (validValues.length) {
    ...
    if (SolveWithBacktracking(steps, id + 1, cells, gridCells, options))
      return true;
...
var GetBestCell = function (cells, gridCells, options) {
  var bestCell = {
    id: -1,
    validValuesCount: options.gridSize * options.gridSize + 1,
  };
...
var GetValidValues = function (cells, cell, options) {
  ...
  var invalidValues = cells
    .filter((c) => CellsConflict(c, cell))
    .map((c) => c.value);
...
var DescribeStep = function (change) {
  ...
  switch (change.state) {
    ...
    case cellStatesEnum.const:
      text = text =
        change.validValuesCount + " valid values, Putting: " + change.newValue;
      break;
  ...
  var color = mainConfig.cellStatesList.find(
    (state) => state.value == change.state
  ).color;
  return { text, color };
};
